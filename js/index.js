$(function () {
    ...
    function startWolfAnimation() {
        ...
        // 随机获取数组类型
        var wolfType = Math.round(Math.random());
        var wolfArr = wolfType == 0 ? wolf_1 : wolf_2;
        // 5.设置图片的内容
        window.wolfIndex = 0; // 全局变量
        window.wolfIndexEnd = 5; // 全局变量
        wolfTimer = setInterval(function () {
            ...
            $wolfImage.attr("src", wolfArr[wolfIndex]);
            wolfIndex++;
        }, 300);
        // 6.将图片添加到界面上
    ...
    function gameRules($wolfImage, wolfType) {
        $wolfImage.one("click", function () { // one表示只执行一次
            // 修改索引
            window.wolfIndex = 5;
